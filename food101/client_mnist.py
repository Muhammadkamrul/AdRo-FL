...
class Client:
    ...
    def train2(self, quantization_bit, epochs=1): 
        self.model.train()
        data_loader = DataLoader(self.data, batch_size=64, shuffle=True)

        total_l2_norm = 0.0
        total_loss = 0.0
        num_total_batches = 0  # Tracks total number of batches across all epochs
        accumulated_gradients = None  # To store cumulative gradients

        for epoch in range(epochs):
            epoch_l2_norm = 0.0  
            epoch_loss = 0.0
            num_batches = 0  # Tracks batches per epoch

            for data, target in data_loader:
                data, target = data.to(self.device, non_blocking=True), target.to(self.device, non_blocking=True)
                self.model.zero_grad()  # Clear gradients from the last batch
                output = self.model(data)
                ...
                # Store gradients for server aggregation
                ...
                for i, grad in enumerate(batch_gradients):
                    accumulated_gradients[i] += grad  # Sum up gradients
                
                # Track batch-level values
                epoch_l2_norm += l2_norm
                epoch_loss += loss.item()
                num_batches += 1
                num_total_batches += 1

            # Compute per-epoch average if multiple epochs
            total_l2_norm += epoch_l2_norm / num_batches
            total_loss += epoch_loss / num_batches

        # Final averaging based on number of epochs
        if epochs == 1:
        ...
        # Normalize accumulated gradients over total number of batches
