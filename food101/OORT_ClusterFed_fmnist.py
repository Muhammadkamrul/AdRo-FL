...
class Server:
    def __init__(self, clusters, device, learning_rate=0.01):
        ...
        self.device = device
        self.global_model = MLP_FMNIST().to(device)
...
def main():
    ...
    results_as_list = os.path.join(result_directory, 'output_fmnist_' + "oort_" + args.selection_mode + '_dirichlet_NEW_NonCluster.txt')
    configs_as_list = os.path.join(result_directory, 'output_fmnist_' + "oort_" + args.selection_mode + '_dirichlet_args_NEW_NonCluster.txt')
