...
def select_clients_with_constraints(selector, num_clients, feasible_clients, cluster_assignments, K, mode='global'):
    selector.round += 1
    if mode == 'global':
...
class Server:
    def __init__(self, clusters, device, learning_rate=0.01):
        ...
        self.device = device
        self.global_model = CNN_Cifar().to(device)
...
def get_gradient_size_bits(gradients):
    total_bits = 0
    for grad in gradients:
        num_elements = grad.numel()
        bits_per_element = grad.element_size() * 8  # element_size() gives bytes
...
def main():
    ...
    for round_idx in range(args.num_rounds):
        ...
        violating_clusters = [cluster_id for cluster_id, count in cluster_counts.items() if 0 < count < 2] #<2 means risk of BSA
        if violating_clusters:
            violation_count_list.append(len(violating_clusters))
            ...
            violation_count_list.append(0)

        print(violation_count_list)
        ...
        bits = sum(get_gradient_size_bits(update['gradients']) for update in client_updates)
        grad_size_bits.append(bits)
    ...
    results_as_list = os.path.join(result_directory, 'output_svhn_' + "oort_global_violation" + '.txt')
    ...
    with open(results_as_list, 'w') as file:
        ...
        file.write(f'violation_count = {violation_count_list}\n')
        file.write(f'grad_size_bits = {grad_size_bits}\n')
